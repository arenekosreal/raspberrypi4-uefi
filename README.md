...
- How to build aarch64 package on x86_64 machine (both systems are Arch Based distributions and you should change working directory to this repository):  
    ...
    Update: Now you can compile aarch64 package directly on x86_64 host by running `makepkg CARCH=aarch64` without docker, we use `uname -m` rather than `$CARCH` to check if you are x86_64 host. Of course you have to install `aarch64-linux-gnu-gcc`. The `Dockerfile` in this repository is used for building Arch Linux package on Ubuntu.
